...
## Integration with Kaiju Inventory Service

```typescript
// src/service/request/v1/import/createImportParse.request.ts
import { Router } from "@juannpz/deno-service-tools";
import { defineSchema, DocumentProcessor } from "@juannpz/kaiju-items-processor";
import type { ExtendedContextVariables, RequestContext } from "../../request.definition.ts";
...
const itemSchema = defineSchema({
    fields: [
        ...
        { name: "item_type_id", type: "string", required: false },
        { name: "supplier_id", type: "string", required: false },
        { name: "current_stock", type: "number", required: false },
        { name: "min_stock", type: "number", required: false },
        { name: "unit_of_measure", type: "string", required: true },
        { name: "arca_iva_aliquot_id", type: "string", required: true },
...
export const createImportParseRequest = Router.post<ExtendedContextVariables>("/import/parse")
    .describe("Parse document into FullItem[] structure")
    .body<ImportBody>()
    .headerParam("Authorization")
    .headerParam<"user_id", string>("user_id")
    .headerParam<"role", string>("role")
    .headerParam<"tenant_id", string>("tenant_id")
    .withVariables<ExtendedContextVariables>()
    .handler(async (context) => {
        const { document, filename, format } = context.body;
        ...
        try {
            const result = await processor.process({
                document,
                format: (format as "excel" | "csv" | "pdf" | "auto") ?? "auto",
                filename,
            });

            return context.c.json(result, 200);
        } catch (error) {
